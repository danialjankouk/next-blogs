...
const PostSlug = ({ blogsData }) => {
  ...
  return (
    <div className="bg-gray-200 container mx-auto h-auto pt-2">
      {blogsData.data.docs.slice(0, 1).map((item) => {
        return (
          <div
            key={item._id}
            className="prose-h1:text-slate-500 prose-headings:font-bold prose-h2:text-slate-600 prose-headings:text-xl prose-p:text-lg"
          >
...
export async function getServerSideProps(ctx) {
  ...
  const { data } = await axios.get(
    `http://localhost:5000/api/posts?limit=10&${queryString.stringify(query)}`
  );
  return {
    props: {
      blogsData: data,
